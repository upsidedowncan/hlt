...
class ChatProvider with ChangeNotifier {
  ...
  Future<void> _saveAiMessagesCache() async {
    ...
    await storage.write(
      key: 'ai_messages_$aiConversationId',
  ...
  Future<void> clearAiConversation() async {
    ...
    try {
      final conversation = await AiChatService.getConversation(
        aiConversationId!,
      );
  ...
  Future<void> _loadConversation() async {
    try {
      _conversation = await ConversationService.getConversation(
        conversationId!,
      );
      _conversationTitle = _conversation?.name ?? 'Chat';
      _participants = await ConversationService.getConversationParticipants(
        conversationId!,
      );

      // Setup realtime subscription for messages
      _messagesSubscription =
          MessageService.subscribeToMessages(conversationId!).listen((
            messages,
          ) async {
            // Detect new messages
            ...
            for (int i = 0; i < _messages.length; i++) {
              ...
              if (existingReadReceipts.containsKey(message.id)) {
                ...
                for (final existingRead in existingReads) {
                  if (!mergedReads.any(
                    (r) => r.userId == existingRead.userId,
                  )) {
          ...
          if (reactionId != null) {
            ...
            for (var i = 0; i < _messages.length; i++) {
              if (_messages[i].reactions.any((r) => r.id == reactionId)) {
                final updatedMessage = await MessageService.getMessage(
                  _messages[i].id,
                );
                if (updatedMessage != null) {
  ...
  void _setupTypingRealtime() {
    ...
    _typingSubscription =
        TypingService.getTypingStatus(conversationId!, currentUserId).listen((
          isTyping,
        ) {
      ...
      callback: (payload) async {
        ...
        if (messageId != null && userId != null && readAt != null) {
          ...
          if (messageIndex != -1) {
            ...
            final existingRead = message.readBy.firstWhereOrNull(
              (r) => r.userId == userId,
            );
            if (existingRead == null) {
              final updatedReadBy = [
                ...message.readBy,
                MessageRead(userId: userId, readAt: DateTime.parse(readAt)),
              ];
    ...
    try {
      ...
      final recentMessages = _messages
          ...
          .map((msg) {
            ...
            return {
              'role':
                  'user', // Treat all chat history as 'user' content for the AI
    ...
    try {
      ...
      if (aiSettings.useDeepMode) {
        ...
        await for (final chunk in stream) {
          if (chunk.done) {
            ...
            if (index != -1) {
              _aiMessages[index] = _aiMessages[index].copyWith(
                ...
                isThinking: false,
                isStreaming: false,
                reasoning_content: accumulatedReasoning.isNotEmpty
          ...
          } else if (chunk.type == StreamingTokenType.reasoning &&
              chunk.token != null) {
          ...
          if (index != -1) {
            _aiMessages[index] = _aiMessages[index].copyWith(
              ...
              reasoning_content: accumulatedReasoning.isNotEmpty
                  ...
                  : null,
              isThinking:
                  false, // Stop thinking animation once we start receiving content
      ...
      if (context.mounted) {
        final aiContent = _aiMessages
            ...
            .last
            .content;
        await _generateConversationTitleIfNeeded(context, content, aiContent);
      }
    } catch (e) {
  ...
  Future<void> startVoiceRecording() async {
    if (_isRecording || isAiConversation)
      return; // Don't allow recording in AI conversations
  ...
  Future<void> _generateConversationTitleIfNeeded(
    ...
    try {
      ...
      try {
        var conversation = await AiChatService.getConversation(
          aiConversationId!,
        );
        if (conversation != null) {
  ...
  Future<void> _markMessagesAsRead() async {
    ...
    try {
      ...
      final messagesRead = await MessageService.markConversationMessagesAsRead(
        conversationId!,
      );
      ...
      if (messagesRead) {
        ...
        for (var i = 0; i < _messages.length; i++) {
          ...
          if (message.senderId != currentUserId &&
            ...
            final updatedReadBy = [
              ...message.readBy,
              MessageRead(userId: currentUserId, readAt: DateTime.now()),
            ];
  ...
  Future<bool> deleteMessage(String messageId) async {
    ...
    try {
      ...
      if (success) {
        ...
        debugPrint(
          '🗑️ Removed $removedCount messages from local list immediately',
        );
        notifyListeners();
      }
      return success;
...
class GlassPlusButton extends StatelessWidget {
  ...
  const GlassPlusButton({
    super.key,
    required this.onPressed,
    this.size = 56.0, // Standard FAB size
  });
      ...
      child: BackdropFilter(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            // Subtle surface gradient like previous screens
    ...
    return Container(
      ...
      child: CustomPaint(
        painter: _IconPatternPainter(
          ...
          secondaryColor: theme.colorScheme.secondary.withOpacity(0.03),
        ),
        child: child,
      ),
    );
...
class _IconPatternPainter extends CustomPainter {
  ...
  final List<Offset> _iconPositions = [];
  final List<Offset> _smallIconPositions = [];

  _IconPatternPainter({required this.color, required this.secondaryColor});

  @override
  void paint(Canvas canvas, Size size) {
    // Generate random positions for icons (mosaic style)
    _generateMosaicPattern(size);

    // Create different icon patterns for variety
    final primaryIcons = [
    ...
    // Draw primary pattern (larger icons)
    final primaryPaint = Paint()
      ..color = color
      ..style = PaintingStyle.fill;

    for (int i = 0; i < _iconPositions.length; i++) {
      final position = _iconPositions[i];
      final icon = primaryIcons[i % primaryIcons.length];
      final size = 28.0 + (i % 3) * 6.0; // Varied sizes: 28, 34, 40
      final rotation =
          (i * 0.5) % (2 * math.pi / 3) - (math.pi / 3); // Random rotation

      _drawIcon(canvas, position, icon, size, primaryPaint, rotation);
    }

    // Draw secondary pattern (smaller, more subtle icons)
    final secondaryPaint = Paint()
      ..color = secondaryColor
      ..style = PaintingStyle.fill;

    for (int i = 0; i < _smallIconPositions.length; i++) {
      final position = _smallIconPositions[i];
      final icon = secondaryIcons[i % secondaryIcons.length];
      final size = 8.0 + (i % 2) * 3.0; // Varied sizes: 8, 11
      final rotation =
          (i * 0.8) % (2 * math.pi / 4) -
          (math.pi / 4); // Different rotation pattern

      _drawIcon(canvas, position, icon, size, secondaryPaint, rotation);
    }

    // Removed abstract shapes - keeping only the clean mosaic icon pattern
  }

  void _generateMosaicPattern(Size size) {
    _iconPositions.clear();
    _smallIconPositions.clear();

    final random = math.Random(42); // Fixed seed for consistent pattern

    // Generate larger icons (primary layer)
    final largeIconCount = ((size.width * size.height) / 3000)
        .floor(); // Increased density for closer icons
    for (int i = 0; i < largeIconCount; i++) {
      double x, y;
      int attempts = 0;

      // Try to avoid overlapping with existing icons
      do {
        x = random.nextDouble() * size.width;
        y = random.nextDouble() * size.height;
        attempts++;
      } while (_isTooClose(Offset(x, y), _iconPositions, 35.0) &&
          attempts < 50);

      if (attempts < 50) {
        _iconPositions.add(Offset(x, y));
      }
    }

    // Generate smaller icons (secondary layer)
    final smallIconCount = ((size.width * size.height) / 1500)
        .floor(); // Much more dense for closer smaller icons
    for (int i = 0; i < smallIconCount; i++) {
      double x, y;
      int attempts = 0;
      ...
      do {
        ...
        y = random.nextDouble() * size.height;
        attempts++;
      } while (_isTooClose(Offset(x, y), _smallIconPositions, 40.0) &&
          attempts < 30);

      if (attempts < 30) {
        _smallIconPositions.add(Offset(x, y));
      }
    }
  }
  ...
  bool _isTooClose(
    ...
    for (final existing in positions) {
      final distance = math.sqrt(
        math.pow(position.dx - existing.dx, 2) +
            math.pow(position.dy - existing.dy, 2),
      );
  ...
  bool shouldRepaint(covariant CustomPainter oldDelegate) {
    if (oldDelegate is _IconPatternPainter) {
...
class ChatScreen extends StatefulWidget {
  ...
  const ChatScreen({super.key, this.conversationId, this.aiConversationId});
...
class _ChatAppBar extends StatelessWidget implements PreferredSizeWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return AppBar(
      ...
      title: Row(
        children: [
          if (provider.isAiConversation) ...[
            ...
            ProfileAvatar(
              avatarUrl:
                  provider.conversation?.avatarUrl ??
                  (provider.participants.isNotEmpty
                      ? provider.participants.first.avatarUrl
                      : null),
              displayName: provider.participants.isNotEmpty
                  ? provider.participants.first.displayName ??
                        provider.participants.first.username ??
                        provider.participants.first.email
                  : 'Unknown',
              size: 32,
              showBorder: false,
            ),
      ...
      actions: [
        // Call button for regular conversations
        if (!provider.isAiConversation && provider.participants.isNotEmpty)
          IconButton(
            ...
            onPressed: () {
              ...
              final currentUserId = Supabase.instance.client.auth.currentUser?.id;
              final otherParticipant = provider.participants.firstWhere(
                (participant) => participant.id != currentUserId,
                orElse: () => provider.participants.first, // Fallback to first if no other found
              );
              debugPrint('ChatScreen: Calling ${otherParticipant.displayName} (${otherParticipant.id}) in conversation ${provider.conversationId}');
              context.go('/call?receiverId=${otherParticipant.id}&conversationId=${provider.conversationId}');
            },
          ),
        if (provider.isAiConversation)
          IconButton(
...
class _MessageGroupWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Align(
      ...
      child: Column(
        crossAxisAlignment: group.isFromMe
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedOpacity(
      ...
      child: Column(
        ...
        children: [
          GestureDetector(
            onDoubleTapDown: (details) => _showReactionPicker(
              context,
              details,
            ), // Long press for menu including reactions
            onLongPressStart: (details) => _showReactionPicker(
              context,
...
Widget _buildMessageContent(
  ...
  final textColor = isMite
      ? Colors.white
      : (isFromMe ? theme.colorScheme.onPrimary : theme.colorScheme.onSurface);
      ...
      return Column(
        ...
        children: [
          ...
          Stack(
            children: [
              _buildCustomText(
                _preprocessMiteMentions(_preprocessMarkdown(displayContent)),
                baseStyle,
              ),
              if (isStreaming)
                Positioned(right: 0, bottom: 0, child: _TypingCursor()),
            ],
          ),
          if (vizCode != null) ...[
...
Widget _buildCustomText(String text, TextStyle baseStyle) {
  ...
  for (final Match match in fontRegex.allMatches(text)) {
    ...
    spans.add(
      TextSpan(text: fontText, style: _getFontStyle(baseStyle, fontName)),
    );
    ...
    default:
      ...
      try {
        ...
        try {
          ...
          final String pascalCase = fontName
              .split(' ')
              .map(
          ...
          try {
            ...
            final String cleanFontName = fontName.replaceAll(
              RegExp(r'[^a-zA-Z]'),
              '',
            );
            ...
            debugPrint(
              'Failed to load font "$fontName" (tried multiple formats): $e3',
            );
...
class CallEventMessageWidget extends StatelessWidget {
  final Message message;
  final bool isFromMe;

  const CallEventMessageWidget({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    // For call events, we use the content field which contains the display text
    String displayText = message.content;
    Color textColor = theme.colorScheme.onSurface.withOpacity(0.7);

    // Determine color based on content
    if (displayText.contains('Missed call')) {
      textColor = Colors.red;
    } else if (displayText.contains('Call answered')) {
      textColor = Colors.green;
    } else if (displayText.contains('Call ended')) {
      textColor = theme.colorScheme.onSurface.withOpacity(0.7);
    } else if (displayText.contains('Call declined')) {
      textColor = Colors.orange;
    } else if (displayText.contains('No answer')) {
                ...
                AnimatedSwitcher(
                  ...
                  child: !isRecording && !provider.isAiConversation
                      ...
                      ? SizedBox(
                          ...
                          child: GlassPlusButton(
                            onPressed: () =>
                                context.push(AppConstants.aiSettingsRoute),
                            size: 48,
                          ),
                        )
